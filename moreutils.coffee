...
_.assign Utils,
			...
			do (layer, targetLayer, direction) ->
				switch direction
					when "left"
						props = ['x']
						lProp = 'maxX'
						distance = targetLayer.x - (layer.maxX)
						getDifference = -> targetLayer.x - distance
					when "right"
						props = ['x', 'width']
						lProp = 'x'
						distance = layer.x - (targetLayer.maxX)
						getDifference = -> targetLayer.maxX + distance
					when "top"
						props = ['y']
						lProp = 'maxY'
						distance = targetLayer.y - (layer.maxY)
						getDifference = -> targetLayer.y - distance
					when "bottom"
						props = ['y', 'height']
						lProp = 'y'
						distance = layer.y - (targetLayer.maxY)
						getDifference = -> targetLayer.maxY + distance
					else
	...
	#
	# @example
	#	Utils.fetch 'loripsum.net/api/10/short/headers', (d) -> print d
	#
	...
	#
	# @example
	#	Utils.fetchJSON 'https://www.refugerestrooms.org:443/api/v1/restrooms/by_date.json?day=5&month=6&year=2012'', (d) -> print d
	#
	...
	# @ param options [String] length The average length of a paragraph
	randomText: (words = 12, sentences = false) ->
		...
		while text.length >= 1
			s = _.pullAt(text, _.range(_.random(3, 5)))
			sentences.push(_.capitalize(s.join(' ').trim()))

		return _.shuffle(sentences).join('. ').trim() + '.'
